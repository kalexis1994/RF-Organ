...
- The rotary speaker is a user-facing module inside RF-Organ. It must be addressable by parameters, MIDI controls, saved state and programs. Keep its DSP internally isolated and testable.
...
- Keep local build growth bounded: use `CARGO_INCREMENTAL=0` for validation builds and do not commit generated packages or build artifacts. Branding PNGs under `package/branding/` are part of the package rather than build output, and are regenerated by `tools/make-artwork.py`.
- Release with `tools/release.sh`. It gates on the same checks as CI and packs a reproducible `.rfplugin`; a release that cannot be packed twice to the same bytes is a bug.
